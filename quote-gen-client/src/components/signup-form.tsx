...
// Kiểu props cho SignupForm: cho phép App truyền hàm để chuyển sang màn hình Đăng nhập
...
const Signin2: React.FC<SignupFormProps> = ({ onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Lưu ý: Server expect 'username' nhưng form có 'fullName', nên cần map lại
      const response = await fetch('http://localhost:5000/api/auth/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        // Server expect 'username' field, không phải 'fullName'
      ...
      if(data.user) {
        ...
        // Sau 2 giây, chuyển sang màn hình đăng nhập để user có thể đăng nhập
      ...
      // Tắt trạng thái loading sau khi hoàn thành (thành công hoặc thất bại)
      setIsLoading(false);
    }
  }
        ...
        <div className="w-full max-w-sm relative">
          ...
          >
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              >
                {isLoading ? 'Đang đăng ký tài khoản' : 'Create account'}
              </button>
            </form>
